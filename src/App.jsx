...
const INIT_TICKETS=[
  {id:"TK-001",title:"شراء قماش قطني",type:"شراء",priority:"عالية",status:"قيد التنفيذ",openedBy:2,currentDept:2,currentAssignee:3,createdAt:"2026-05-20",dueDate:"2026-06-10",
   transfers:[{from:2,to:3,dept:2,note:"محتاج اعتماد ميزانية",time:"09:30"}],
   chat:[{uid:2,text:"نحتاج 500 متر قماش قطني درجة أولى",time:"09:00",type:"msg"},{uid:2,text:"تحويل للمحاسبة",time:"09:30",type:"transfer",to:3,toDept:2},{uid:3,text:"جاري مراجعة الميزانية",time:"10:30",type:"msg"}]},
  {id:"TK-002",title:"صيانة ماكينة الخياطة",type:"صيانة",priority:"عاجلة",status:"لم تبدأ",openedBy:4,currentDept:3,currentAssignee:4,createdAt:"2026-05-22",dueDate:"2026-05-30",
   transfers:[],
   chat:[{uid:4,text:"الماكينة رقم 3 تحتاج صيانة عاجلة",time:"08:00",type:"msg"}]},
  {id:"TK-003",title:"استفسار عميل #NJ-101",type:"استفسار",priority:"متوسطة",status:"قيد التنفيذ",openedBy:6,currentDept:4,currentAssignee:5,createdAt:"2026-05-23",dueDate:"2026-06-01",
   transfers:[],
   chat:[{uid:6,text:"متى يكون الطلب جاهز؟",time:"11:00",type:"msg"},{uid:5,text:"نتوقع الجاهزية خلال أسبوع",time:"11:45",type:"msg"}]},
  {id:"TK-004",title:"توريد أقمشة صيفية",type:"شراء",priority:"منخفضة",status:"مكتملة",openedBy:2,currentDept:1,currentAssignee:2,createdAt:"2026-05-10",dueDate:"2026-05-25",
   transfers:[],
   chat:[{uid:2,text:"تم استلام البضاعة بالكامل",time:"14:00",type:"msg"}]}
];
...
export default function App(){
  ...
  const visible=isAdmin?tickets:isClient
    ?tickets.filter(t=>t.openedBy===currentUser.id)
    :tickets.filter(t=>t.openedBy===currentUser.id||t.currentAssignee===currentUser.id||t.currentDept===currentUser.dept);
  ...
  function sendFile(e){
    ...
    reader.onload=ev=>{
      const updated=tickets.map(t=>t.id===selectedTicket.id?{...t,chat:[...t.chat,{uid:currentUser.id,text:file.name,time,type:isImg?"image":"file",data:ev.target.result,fileType:file.type}]}:t);
      setTickets(updated);setSelectedTicket(updated.find(t=>t.id===selectedTicket.id));
    };
    isImg?reader.readAsDataURL(file):reader.readAsDataURL(file);
    e.target.value="";
  }
  ...
  function doTransfer(){
    ...
    const now=new Date();const time=`${now.getHours().toString().padStart(2,"0")}:${now.getMinutes().toString().padStart(2,"0")}`;
    const toUser=USERS.find(u=>u.id===transferUser);
    const toDept=DEPTS.find(d=>d.id===transferDept);
    const transferEntry={from:currentUser.id,to:transferUser,dept:transferDept,note:transferNote,time};
    const chatEntry={uid:currentUser.id,text:`تحويل إلى ${toUser?.name} (${toDept?.name})${transferNote?` — ${transferNote}`:""}`,time,type:"transfer",to:transferUser,toDept:transferDept};
    const updated=tickets.map(t=>t.id===selectedTicket.id?{...t,currentAssignee:transferUser,currentDept:transferDept,status:"قيد التنفيذ",transfers:[...t.transfers,transferEntry],chat:[...t.chat,chatEntry]}:t);
    setTickets(updated);setSelectedTicket(updated.find(t=>t.id===selectedTicket.id));
  ...
  const navItems=[{id:"dashboard",icon:"🏠",label:"الرئيسية"},{id:"tickets",icon:"🎫",label:"التذاكر"},...(!isClient?[{id:"my",icon:"✅",label:"مهامي"}]:[]),...(isAdmin?[{id:"team",icon:"👥",label:"الفريق"}]:[])];
  ...
  const topTitle=selectedTicket?`← ${selectedTicket.id}`:{dashboard:"لوحة التحكم",tickets:"التذاكر",my:"مهامي",team:"الفريق"}[page];
        ...
        <div style={{flex:1,overflow:"auto",padding:22}}>
          ...
          {page==="dashboard"&&<div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
              ...
              <div className="card" style={{padding:18}}>
                ...
                {DEPTS.map(d=>{const cnt=tickets.filter(t=>t.currentDept===d.id).length;const done=tickets.filter(t=>t.currentDept===d.id&&t.status==="مكتملة").length;const pct=cnt?Math.round(done/cnt*100):0;return(<div key={d.id} style={{marginBottom:12}}><div style={{display:"flex",justifyContent:"space-between",fontSize:12,marginBottom:4}}><span>{d.icon} {d.name}</span><span style={{fontWeight:700,color:d.color}}>{cnt} تذاكر</span></div><div style={{height:5,background:"#1e1e24",borderRadius:3}}><div style={{height:"100%",width:`${pct}%`,background:d.color,borderRadius:3}}/></div></div>);})}
              </div>
            </div>
          </div>}
          ...
          {page==="tickets"&&!selectedTicket&&<div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:14}}>
            {["لم تبدأ","قيد التنفيذ","مكتملة"].map(st=>{const cols=visible.filter(t=>t.status===st);return(<div key={st}>
              ...
              {cols.map(t=>{const op=USERS.find(u=>u.id===t.openedBy);const as=USERS.find(u=>u.id===t.currentAssignee);const dept=DEPTS.find(d=>d.id===t.currentDept);return(<div key={t.id} className="kan-card" onClick={()=>setSelectedTicket(t)}>
                ...
                <div style={{fontSize:11,color:dept?.color,marginBottom:8}}>{dept?.icon} {dept?.name}</div>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 320px",gap:14}}>
                ...
                <div style={{display:"flex",flexDirection:"column",gap:12}}>
                  <div className="card" style={{padding:14}}>
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}><Av user={as} size={36}/><div><div style={{fontSize:11,color:"#71717a"}}>المسؤول الحالي</div><div style={{fontSize:13,fontWeight:600}}>{as?.name}</div><div style={{fontSize:11,color:dept?.color,marginTop:2}}>{dept?.icon} {dept?.name}</div></div></div>
                  ...
                  {t.transfers.length>0&&<div className="card" style={{padding:14}}>
                    ...
                    {t.transfers.map((tr,i)=>{const fromU=USERS.find(u=>u.id===tr.from);const toU=USERS.find(u=>u.id===tr.to);const toDept=DEPTS.find(d=>d.id===tr.dept);return(<div key={i} style={{marginBottom:10,paddingBottom:10,borderBottom:i<t.transfers.length-1?"1px solid #1e1e24":"none"}}>
                      <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:4}}>
                        <Av user={fromU} size={20}/><span style={{fontSize:11,color:"#71717a"}}>→</span><Av user={toU} size={20}/><span style={{fontSize:11,fontWeight:600}}>{toU?.name}</span>
                      </div>
                      <div style={{fontSize:10,color:toDept?.color}}>{toDept?.icon} {toDept?.name}</div>
      ...
      {showTransfer&&<div onClick={()=>setShowTransfer(false)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,.5)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:50}}>
        <div onClick={e=>e.stopPropagation()} style={{background:"#13131a",border:"1px solid #27272a",borderRadius:16,padding:24,width:360}}>
          ...
          <div style={{marginBottom:12}}><div style={{fontSize:11,color:"#71717a",marginBottom:5}}>القسم</div>
            <select className="form-inp" value={transferDept} onChange={e=>{setTransferDept(parseInt(e.target.value));setTransferUser(USERS.find(u=>u.dept===parseInt(e.target.value)&&u.type==="employee")?.id||2);}}>
          ...
          <div style={{marginBottom:18}}><div style={{fontSize:11,color:"#71717a",marginBottom:5}}>ملاحظة (اختياري)</div>
            <textarea className="form-inp" rows={3} placeholder="سبب التحويل أو تعليمات..." value={transferNote} onChange={e=>setTransferNote(e.target.value)} style={{resize:"none"}}/>
          </div>
